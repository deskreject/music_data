...
# CONFIGURATION
# ------------------------------------------------------------------------------
SPOTIFY_CLIENT_ID = "a8342ecb25754af881ee95a592646bb0"
SPOTIFY_CLIENT_SECRET = "b1f1358f43b14949bb3ddfdc9d9a0b6e"

INPUT_DIR = Path(r"Z:\\Data_alexander\\data\\raw_data\\Spotify\\1980_2000_songs_artists\\parts_charted_songs\\more_normalized")
...
def search_track(artist_name, track_name, max_retries=5):
    ...
    query_fallback = f"{track_name} {artist_name}"  # fallback simple query
    wait_seconds = 5
    ...
    for attempt in range(max_retries):
        try:
            ...
            # --- Fallback query (top 5 results) ---
            result = sp.search(q=query_fallback, type="track", limit=5)
            items = result.get("tracks", {}).get("items", [])

            best_score = 0
            best_track = None
            for item in items:
                sp_artist = normalize(", ".join([a["name"] for a in item["artists"]]))
                sp_track = normalize(item["name"])
                score = fuzz.token_sort_ratio(f"{norm_track} {norm_artist}", f"{sp_track} {sp_artist}")
                if score > best_score:
                    best_score = score
                    best_track = item

            if best_score >= 90:
                return extract_track_info(best_track)
...
if __name__ == "__main__":
    file = INPUT_DIR / "not_found_part5.csv"  # Adjust per script instance
