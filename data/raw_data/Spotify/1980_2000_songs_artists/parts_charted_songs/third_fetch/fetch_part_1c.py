# -*- coding: utf-8 -*-
"""
Created on Thu Jul  3 19:11:29 2025
...
# CONFIGURATION
# ------------------------------------------------------------------
SPOTIFY_CLIENT_ID = "1f63f3fa94e9452ba32e214fce1bbd03"
SPOTIFY_CLIENT_SECRET = "539764e256cc447dbf290faa8eaf991e"

INPUT_DIR = Path(r"Z:\\Data_alexander\\data\\raw_data\\Spotify\\1980_2000_songs_artists\\parts_charted_songs\\more_normalized\\third_fetch")
...
def get_market(country):
    """Map input country to Spotify's market code."""
    if not isinstance(country, str):
        return None
    code = country.strip().lower()
    if code == 'uk':
        return 'GB'
    allowed = ['us', 'it', 'gb', 'de', 'fr']
    if code in allowed:
        return code.upper()
    return None
...
# ------------------------------------------------------------------
def search_track(artist_name, track_name, market=None, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if items:
                return extract_track_info(items[0])
            return None
...
def process_csv_file(file_path):
    ...
    for start in range(0, len(df_part), CHUNK_SIZE):
        ...
        for i, row in df_chunk.iterrows():
            ...
            market = get_market(row.get("country", ""))

            if not artist or not track or not market:
                continue

            key = (track, artist, market)
            if key in cache:
                track_info = cache[key]
            else:
                track_info = search_track(artist, track, market=market)
...
if __name__ == "__main__":
    file = INPUT_DIR / "missing_after_second_fetch_part1.csv"  # Adjust per script instance
